...
async def get_favorites(user=Depends(get_current_user)):
    """Возвращает список избранных парков текущего пользователя"""
    conn = get_connection()
    try:
        rows = conn.execute(
...
async def add_favorite(park_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        # Вставляем, игнорируем дубликаты
        conn.execute(
...
async def get_user_stats(user=Depends(get_current_user)):
    """Возвращает статистику пользователя: количество оценок и уровень"""
    conn = get_connection()
    try:
        row = conn.execute(
        ...
        level = (photo_votes_count // 10) + 1  # 1 уровень за каждые 10 оценок
